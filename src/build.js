...
// through __STYLE__. ----
// ---- the fein tile ----
// logo.svg is the font-free vector of the master (logo.png, the file the brand
// was supplied as): the red tile and the wordmark as one path, regenerated by
// scripts/logo-svg.py. Everything that shows the mark derives from it here, so
// the tab, the nav, the footer, the bot avatar and the link preview cannot
// drift apart: __BRAND_MARK__ in the template and the pages inlines it,
// favicon.svg is the same file with rounded corners, og.svg embeds the path,
// and scripts/icons.mjs rasterises it for favicon.ico and the maskable icon.
const LOGO_SVG = fs.readFileSync("logo.svg", "utf8");
const LOGO_PATH = (LOGO_SVG.match(/<path[^>]*\/>/) || [])[0];
const LOGO_RED = (LOGO_SVG.match(/<rect[^>]*fill="(#[0-9A-Fa-f]{6})"/) || [])[1];
if (!LOGO_PATH || !LOGO_RED) throw new Error("logo.svg: tile rect or wordmark path not found");
const BRAND_MARK = `<svg class="mark" viewBox="0 0 1080 1080" aria-hidden="true" focusable="false"><rect width="1080" height="1080" fill="${LOGO_RED}"/>${LOGO_PATH}</svg>`;
const tplSrc = fs.readFileSync("fein.tpl.html", "utf8").split("__BRAND_MARK__").join(BRAND_MARK);
...
if (body.indexOf("__BRAND_MARK__") > -1) throw new Error("brand mark placeholder left");
...
const ld = {
  ...
  "@graph": [
    ...
    { "@type": "Organization", "@id": SITE + "/#org", name: "fein", alternateName: "Fein", url: SITE + "/", description: DESC_LONG, slogan: "the memory layer for private capital", email: "sales@fein.vc", logo: SITE + "/logo.png", image: SITE + "/og.png", sameAs: [REPO_URL], knowsAbout: ["venture capital", "private equity", "limited partner relations", "relationship intelligence", "entity resolution", "knowledge graphs", "Model Context Protocol"], contactPoint: { "@type": "ContactPoint", contactType: "sales", email: "sales@fein.vc", url: SITE + "/demo" } },
...
const ORG_LITE = { "@type": "Organization", "@id": SITE + "/#org", name: "fein", url: SITE + "/", logo: SITE + "/logo.png", email: "sales@fein.vc", sameAs: [REPO_URL] };
...
["privacy", "terms"].forEach(function (slug) {
  let page = fs.readFileSync(path.join("legal", slug + ".html"), "utf8")
    ...
    .split("__BRAND_MARK__").join(BRAND_MARK);
...
["private-equity", "fundraising", "portfolio", "demo", "self-host", "deploy", "pricing", "security", "integrations"].forEach(function (slug) {
  ...
  if (page.indexOf("__STYLE__") > -1 || page.indexOf("__LOGO_SPRITE__") > -1 || page.indexOf("__NAV") > -1 || page.indexOf("__BRAND_MARK__") > -1) throw new Error("page placeholder left in " + slug);
...
// favicon.svg — the tile with 12% rounded corners, the way an icon sits in a
// tab; logo.svg — the square tile as supplied, for anything that wants the
// vector (email signatures, decks, press). Both are the same path.
const FAVICON_SVG = LOGO_SVG.replace('<rect width="1080" height="1080"', '<rect width="1080" height="1080" rx="130"');
if (FAVICON_SVG === LOGO_SVG) throw new Error("logo.svg: tile rect not found for favicon.svg");
...
<g transform="translate(92,88) scale(${(176 / 1080).toFixed(6)})"><rect width="1080" height="1080" rx="130" fill="${LOGO_RED}"/>${LOGO_PATH}</g>
<text x="92" y="366" fill="#ffffff" font-size="56" font-weight="600" letter-spacing="-1.5">The memory layer for private capital.</text>
<text x="92" y="420" fill="#8f8f8f" font-size="26" font-weight="400">Warm intros · meeting prep · deal memory, inside Claude &amp; ChatGPT</text>
<text x="92" y="546" fill="#ededed" font-size="26" font-weight="500">fein.vc</text>
<text x="1108" y="546" fill="#6f6f6f" font-size="20" font-weight="500" letter-spacing="4" text-anchor="end">OPEN SOURCE · SELF-HOSTED</text>
</g>
</svg>
`);
